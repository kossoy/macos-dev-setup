...
|----------|---------|---------|
| `WORK_CONTEXT_NAME` | Display name | `"SAP Concur"` |
| `WORK_ORG` | Organization ID | `"CONCUR"` |
...
graph TD
    ...
    I --> K{Work key exists?}
    J --> K
    K -->|Yes| L[Load id_ed25519_concur]
...
1. **SSH Key Management:**
   - **Unloads** work key (`id_ed25519_concur`)
...
| Work | `~/.ssh/id_ed25519_concur` | Work SSH key (loaded in work context) |
...
# Unload key
ssh-add -d "$HOME/.ssh/id_ed25519_concur"
```
